...
CREATE TABLE public.connector_bundle (
    id integer NOT NULL,
...
ALTER TABLE public.connector_bundle ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public.connector_bundle_id_seq
...
CREATE TABLE public.bundle_version (
    id integer NOT NULL,
    connector_bundle_id integer NOT NULL,
...
ALTER TABLE public.bundle_version ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public.bundle_version_id_seq
    START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
);
...
ALTER TABLE ONLY public.connector_bundle
    ADD CONSTRAINT connector_bundle_pkey PRIMARY KEY (id);

ALTER TABLE ONLY public.bundle_version
...
CREATE INDEX idx_implver_bver ON public.implementation_version USING btree (bundle_version_id);
...
CREATE INDEX idx_impl_bundle ON public.implementation USING btree (connector_bundle_id);

CREATE INDEX idx_bver_bundle ON public.bundle_version USING btree (connector_bundle_id);

CREATE INDEX idx_bver_connid ON public.bundle_version USING btree (connid_version);
...
ALTER TABLE ONLY public.implementation
    ADD CONSTRAINT fk_impl_bundle FOREIGN KEY (connector_bundle_id) REFERENCES public.connector_bundle(id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED;
...
ALTER TABLE ONLY public.bundle_version
    ADD CONSTRAINT fk_bver_bundle FOREIGN KEY (connector_bundle_id) REFERENCES public.connector_bundle(id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED;

ALTER TABLE ONLY public.bundle_version
    ADD CONSTRAINT fk_bver_connid FOREIGN KEY (connid_version) REFERENCES public.connid_version(version) DEFERRABLE INITIALLY DEFERRED;

ALTER TABLE ONLY public.implementation_version
    ADD CONSTRAINT fk_implver_bver FOREIGN KEY (bundle_version_id) REFERENCES public.bundle_version(id) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED;
